...
% Let's suppose your training data is now loaded in X_train , Y_train
...
for alpha = [0.5 0.1 0.2 0.3 0.7 0.8 0.9 1 0] % Different alpha parameters
    [ranking, ~] = infFS( X_train , Y_train, alpha , 0 ,flag, 0 , filename);
    X_train_ = sparse(double(X_train(:,ranking(1:fixed_feats))));
    cmd = ['-q -s ',num2str(0),' -c ', num2str(1)];
    model = train(Y_train, X_train_, cmd);
    % Testing
    [pred , ~, ~] = predict(Y_train,  X_train_, model ,'-q');
    cv = sum(pred==Y_train)/length(Y_train);
    if (cv > bestcv)
        bestcv = cv;
        best_a = alpha;
        if bestcv>=0.99999
...
[ranking, w] = infFS( X_train , Y_train, best_a , 0 ,flag, 0,filename );
