function [RANKED, WEIGHT] = infFS( X_train, Y_train, alpha, supervision,loading, verbose, filename )
% [RANKED, WEIGHT ] = infFS( X_train, Y_train, alpha, supervision,loading, verbose, filename ) computes ranks and weights
% of features for input data matrix X_train and labels Y_train using Inf-FS algorithm.
%
% Version 6.1, July 2018.
%
% INPUT:
%
...
%  ------------------------------------------------------------------------
fprintf('\n+ Feature selection method: inf-FS 2018\n');
...
r = (0.99/max(eig( A ))); % Set a meaningful value for r
